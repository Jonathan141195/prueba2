...
export function EditarUsuario() {
  ...
  // Obtener datos del usuario si se proporciona un ID válido
  useEffect(() => {
    if (id !== undefined && !isNaN(Number(id))) {
      UsuarioService.getUserById(Number(id))
        .then((response) => {
        ...
        UsuarioService.updateUsuario(DataForm)
          .then((response) => {
            ...
            if (response.data.results != null) {
              ...
              toast.error('Error al actualizar el usuario', { duration: 4000, position: 'top-center' });
            }
          })
          .catch((error) => {
            console.log(error);
  ...
  <FormControl variant="standard" fullWidth>
    ...
    />
          ...
          <Grid item xs={12} mt={2}>
            <Button type="submit" variant="contained">Actualizar Usuario</Button>
          </Grid>
        </Grid>
      </form>
    </>
